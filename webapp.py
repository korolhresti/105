...
# Виправлено імпорт для Aiogram v3+
...
def escape_markdown_v2(text: str) -> str:
    ...
    # Використовуємо реплейсмент, щоб уникнути проблем з `\` в f-стрінгах
    # Зауваження: `.` є екранованим у MarkdownV2, але лише після пробілу або на початку/кінці речення
    # Для простоти екрануємо завжди.
    special_chars = [
        '_', '*', '[', ']', '(', ')', '~', '`', '>', '#', '+',
        '-', '=', '|', '{', '}', '.', '!'
    ]
    ...
    for char in special_chars:
        # Екрануємо символ, якщо він не є частиною вже екранованої послідовності
...
@app.get("/")
def healthcheck():
    return {"status": "ok"}  # 👈 обовʼязково додай
